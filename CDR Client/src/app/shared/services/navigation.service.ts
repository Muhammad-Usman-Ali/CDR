...
export class NavigationService {
    ...
    // for normal use everywhere
    defaultMenu: IMenuItem[] = [];

    // used for filtering
    private _defaultMenu: IMenuItem[] = [
        {
          for: { production: true, admin: false },
          name: 'Dashboard',
        ...
        {
          for: { production: true, admin: true },
          name: 'Users',
        ...
        {
          for: { production: false, admin: false },
            name: 'UI kits',
    ...
    menuItems = new BehaviorSubject<IMenuItem[]>(this.publishNavigationChange(true));
    // navigation component has subscribed to this Observable
    menuItems$ = this.menuItems.asObservable();

    // You can customize this method to supply different menu for
    // different user type.
    publishNavigationChange(_isFirstTime: boolean) {

      // if env is on
      if (environment.production) {
        this.defaultMenu = this._defaultMenu.filter(_e => _e.for.production);

        this.filterDefaultMenuArray(this.defaultMenu);
      } else {
        this.filterDefaultMenuArray(this._defaultMenu);
      }
    ...
    filterDefaultMenuArray(_array: any[]): void {
      // console.log(this.auth.getUserDetails().isAdmin);
      // if not admin
      if (!this.auth.getUserDetails().isAdmin) {
        this.defaultMenu = _array.filter(_e => !_e.for.admin);
      } else {

        // fiter for creating a new copy of object reference
        this.defaultMenu = _array.filter(_e => true);
      }
    }
}
